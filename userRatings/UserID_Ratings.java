...
public class UserID_Ratings extends DBIndex {
	...
	public UserID_Ratings(DBBuffer _buffer, String filename, int isCreate) throws IOException {
		...
		if(isCreate == 1) {
			...
			lastPage = new UserID_RatingsPage(pageSize);
		}
		else
	 ...
	 * @throws IOException
	 */
	protected UserID_RatingsPage myReadPage( int pageID ) throws IOException {
		UserID_RatingsPage page;
		DBBufferReturnElement ret = buffer.readPage(file, pageID);
		if ( ret.parsed ) {
			page = (UserID_RatingsPage)ret.object;
		}
		else {
			page = new UserID_RatingsPage(pageSize);
	 ...
	 * @throws IOException
	 */
	public int insertEntry (UserRatings key) throws IOException {
